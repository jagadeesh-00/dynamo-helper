...
exports.buildQueryTableParams = exports.keyOperatorLookup = void 0;
...
exports.keyOperatorLookup = keyOperatorLookup;
/**
 ...
 */
function validateInput(filter, partitionKeyName, sortKeyName) {
...
function buildConditionExpressions(where, partitionKeyName, sortKeyName) {
        ...
        if (key === partitionKeyName) {
            ...
            if (condition && condition.constructor.name === 'Object') {
                ...
                if (operator === 'BETWEEN') {
                    ...
                    keyConditionExpression.push(`${keyName} ${operator} ${valueExpression}_start` +
                        ` AND ${valueExpression}_end`);
                }
                else if (operator === 'BEGINS_WITH') {
                    expressionAttributeValues[valueExpression] = condition;
                    keyConditionExpression.push(`${operator.toLowerCase()}(${keyName}, ${valueExpression})`);
                }
                else {
                    expressionAttributeValues[valueExpression] = condition;
                    keyConditionExpression.push(`${keyName} ${operator} ${valueExpression}`);
                }
            }
                ...
                // eslint-disable-next-line prefer-destructuring
                expressionAttributeValues[`${valueExpression}_end`] = condition[1];
                keyConditionExpression.push(`${keyName} BETWEEN ${valueExpression}_start` +
                    ` AND ${valueExpression}_end`);
            }
            else {
                expressionAttributeValues[valueExpression] = condition;
            ...
            if (operator === 'EXISTS') {
                ...
                expressionAttributeValues[valueExpression] = condition;
                filterExpression.push(`${operator.toLowerCase()}(${keyName}, ${valueExpression})`);
            }
            else {
                expressionAttributeValues[valueExpression] = condition;
 ...
 */
function buildQueryTableParams(filter, partitionKeyName = 'pk', sortKeyName = 'sk') {
    ...
    const tableParams = buildConditionExpressions(filter.where, partitionKeyName, sortKeyName);
    ...
    if (filter.orderBy) {
        tableParams.ScanIndexForward = filter.orderBy === types_1.Direction.ASC;
    }
    return tableParams;
}
exports.buildQueryTableParams = buildQueryTableParams;
