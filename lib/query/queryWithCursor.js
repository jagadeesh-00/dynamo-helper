...
exports.queryWithCursor = void 0;
 ...
 */
async function queryWithCursor(dbClient, table, filter, indexName) {
    ...
    if (!table.cursorSecret) {
        throw new Error('Expected `cursorSecret` which is used to encrypt the `LastEvaluatedKey`');
    }
    ...
    params.ExclusiveStartKey = utils_1.decrypt(filter.prevCursor, table.cursorSecret);
    ...
    return {
        items: items,
        cursor: utils_1.encrypt(output.LastEvaluatedKey, table.cursorSecret),
